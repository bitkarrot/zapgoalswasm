(() => {
  ...
  const defaultGoal = walletId => ({walletId: walletId || null, title: '', descriptionAbove: '', descriptionBelow: '', goalAmount: 10000, targetDate: '', suggestedAmounts: [21, 100, 500, 1000], walletMode: 'vanilla', backgroundColor: '#FFFFFF', textColor: '#1F2937', progressColor: '#F59E0B', remainderColor: '#E5E7EB', fontName: 'sans-serif', fontWeight: 400, nostrPubkey: '', lightningAddressUsername: '', currentAmount: 0})
  const app = Vue.createApp({
    render: window.ZAPGOALS_INDEX_RENDER(),
    data: () => ({goals: [], wallets: [], loading: false, loadError: '', saving: false, formError: '', isDark: false, formDialog: {show: false, editing: false, data: defaultGoal(null)}, deleteDialog: {show: false, loading: false, goal: null}, modeOptions: [{label: 'Vanilla invoice only', value: 'vanilla'}, {label: 'Bitcoin Connect', value: 'all'}], fontOptions: ['sans-serif', 'system-ui, sans-serif', 'Arial, sans-serif', '"Trebuchet MS", sans-serif', 'Verdana, sans-serif', 'Tahoma, sans-serif', 'serif', 'Georgia, serif', '"Times New Roman", serif', 'monospace', '"Courier New", monospace'].map(value => ({label: value.replace(/,.*$/, '').replaceAll('"', ''), value})), fontWeightOptions: [{label: 'Regular', value: 400}, {label: 'Semi-bold', value: 600}, {label: 'Bold', value: 700}, {label: 'Extra-bold', value: 800}] }),
    computed: {
    ...
    methods: {
      ...
      openGoalDialog(goal = null) { const firstWallet = this.wallets[0]?.id || null; this.formError = ''; this.formDialog = {show: true, editing: Boolean(goal), data: goal ? {...goal, targetDate: this.toLocalDateTime(goal.targetDate), suggestedAmounts: JSON.parse(goal.suggestedAmounts || '[]').concat([null, null, null, null]).slice(0, 4)} : defaultGoal(firstWallet)} },
      ...
      validNostr(value) { return !value || /^[0-9a-fA-F]{64}$/.test(value) || 'Enter exactly 64 hexadecimal characters.' },
      validUsername(value) { return !value || /^[a-z0-9._-]{1,64}$/.test(value) || 'Enter a valid lowercase username.' },
      async saveGoal() { const data = this.formDialog.data; if (!data.title?.trim() || !data.walletId || !Number.isInteger(Number(data.goalAmount)) || Number(data.goalAmount) < 1 || !data.targetDate) { this.formError = 'Complete the required fields with valid values.'; return; } const amounts = data.suggestedAmounts.filter(value => value !== null && value !== '').map(Number); if (!amounts.length || amounts.some(value => !Number.isInteger(value) || value < 1) || new Set(amounts).size !== amounts.length) { this.formError = 'Configure one to four unique whole-satoshi amounts.'; return } const payload = {...data, title: data.title.trim(), targetDate: new Date(data.targetDate).toISOString(), suggestedAmounts: amounts, fontWeight: Number(data.fontWeight), nostrPubkey: data.nostrPubkey?.trim().toLowerCase() || '', lightningAddressUsername: data.lightningAddressUsername?.trim().toLowerCase() || ''}; this.saving = true; this.formError = ''; try { const saved = await this.api(this.formDialog.editing ? 'PUT' : 'POST', this.formDialog.editing ? `/goals/${data.id}` : '/goals', payload); const index = this.goals.findIndex(goal => goal.id === saved.id); if (index < 0) this.goals.unshift(saved); else this.goals.splice(index, 1, saved); this.closeGoalDialog(); LNbitsBridge.notify('Goal saved.', 'positive').catch(() => {}) } catch (error) { this.formError = error.message } finally { this.saving = false } },
      ...
      toLocalDateTime(value) { if (!value) return ''; const date = new Date(value); return new Date(date.getTime() - date.getTimezoneOffset() * 60000).toISOString().slice(0, 16) },
